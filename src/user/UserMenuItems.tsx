...
export const UserMenuItems: React.FC<UserMenuItemsProps> = ({ setMobileMenuOpen }) => {
  ...
  return (
    <div className="bg-boxdark2">
      {/* User Info Section */}
      <div className="py-2.5 pl-10 pr-10 border-b border-graydark">
        <div className="flex items-center gap-5">
          <div className="font-semibold text-primary text-xl text-left">
            {userData?.username?.split('@')[0] || userData?.email?.split('@')[0]}
          </div>
        </div>
      </div>
      ...
      {/* Menu Links */}
      <ul
        className={cn('flex flex-col gap-5 text-white py-4', {
          'sm:px-6': path !== adminDashboardPath,
          'px-6': path === adminDashboardPath,
        })}
      >
        <li className="flex gap-5 text-sm font-medium duration-300 ease-in-out hover:text-primary">
          <Link to={routes.AccountRoute.to} onClick={handleMobileMenuClick} className="flex gap-5">
            <svg
      ...
      {!!userData && userData.isAdmin && (
        ...
        >
          <li className="flex gap-5 text-sm font-medium duration-300 ease-in-out hover:text-primary">
            <Link to={routes.AdminRoute.to} onClick={handleMobileMenuClick} className="flex gap-5">
              <svg
                className="fill-white"
                width="18"
                height="18"
                ...
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M3 4a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1H4a1 1 0 01-1-1V4zm12 0a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1V4zM3 14a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1H4a1 1 0 01-1-1v-4zm12 0a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1v-4z" />
              </svg>
              Admin Dashboard
            </Link>
          </li>
      ...
      <button
        onClick={() => logout()}
        className={cn(
          'flex gap-5 py-4 w-full text-sm font-medium duration-300 ease-in-out border-t border-graydark hover:text-primary',
          {
            'sm:px-6': path !== adminDashboardPath,
            'px-6': path === adminDashboardPath,
          }

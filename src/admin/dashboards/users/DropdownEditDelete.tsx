    ...
    <div className='relative'>
      ...
      <div
        ...
        onKeyDown={(e) => {
          ...
          if (e.key === 'Enter') setDropdownOpen(true);
        }}
        onFocus={() => setDropdownOpen(true)}
        onBlur={() => setDropdownOpen(false)}
        className={cn(
          'absolute right-0 top-full z-40 w-40 space-y-1 rounded-sm border border-bodydark p-1.5 shadow-default border-boxdark bg-boxdark',
          {
            block: dropdownOpen,
            hidden: !dropdownOpen,
          }
      ...
      >
        <button className='flex w-full items-center gap-2 rounded-sm py-1.5 px-4 text-left text-sm hover:bg-gray hover:bg-graydark'>
          <svg
